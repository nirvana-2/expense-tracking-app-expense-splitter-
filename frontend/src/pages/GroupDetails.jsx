        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white pb-20">
            ...
            <header className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/5">
                <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                    <button
                        onClick={() => navigate('/')}
                        className="p-2 -ml-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg transition-colors"
                    >
                        <ArrowLeft size={20} />
                    </button>
            ...
            <main className="container mx-auto px-4 pt-6 max-w-4xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200 dark:border-white/5">
                        <div className="text-slate-500 dark:text-slate-400 text-sm mb-1">Total Expenses</div>
                        <div className="text-2xl font-bold text-slate-900 dark:text-white">
                            Rs. {expenses.reduce((sum, start) => sum + start.amount, 0).toFixed(2)}
                        </div>
                    </div>

                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200 dark:border-white/5 relative group">
                        ...
                        {showMembersDropdown && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                <div className="p-2 max-h-60 overflow-y-auto space-y-1">
                                    {group.members.map(member => {
                                        ...
                                        return (
                                            <div key={member._id} className="flex items-center justify-between p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg group/member">
                                                <div className="flex items-center gap-2 overflow-hidden">
                                                    ...
                                                    <span className="text-sm text-slate-700 dark:text-slate-300 truncate max-w-[100px]">{member.name}</span>
                                                </div>
                ...
                <div>
                    ...
                    <div className="space-y-3">
                        {expenses.length === 0 ? (
                            <div className="text-center py-10 bg-slate-100 dark:bg-white/5 rounded-2xl border border-dashed border-slate-300 dark:border-white/10">
                            ...
                            expenses.map(expense => (
                                <div
                                    key={expense._id}
                                    className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-white/5 hover:border-indigo-500/30 transition-colors cursor-pointer group shadow-sm dark:shadow-none"
                                >
                                        ...
                                        <div className="text-right">
                                            ...
                                            {(group.admin === user._id || expense.paidBy._id === user._id) && (
                                                <button
                                                    ...
                                                    className="inline-flex p-1 text-slate-400 dark:text-slate-500 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors"
                                    ...
                                    {(() => {
                                        ...
                                        if (mySplit) {
                                            return (
                                                <div className="mt-2 text-xs bg-slate-100 dark:bg-black/20 inline-block px-2 py-1 rounded-lg text-slate-500 dark:text-slate-400">
                                                    Your share: <span className="text-rose-500 dark:text-rose-400">Rs. {mySplit.amount.toFixed(2)}</span>
                                                </div>
                                            );
